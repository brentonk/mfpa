...
```{r get_pres_data}
#| message: false
# Placeholder: synthetic, countypres-shaped data generated offline (see
# data/make_synthetic_county_pres.R). These are fabricated numbers, not real
# election returns; they stand in for a Harvard Dataverse download so the book
# renders deterministically with no network dependency. To be reworked.
df_pres_county <- read_csv("data/synthetic_county_pres.csv", show_col_types = FALSE)
```
...
These tools are known as [differential calculus]{.concept definition='Mathematical techniques for studying the "rate of change" of a function --- how quickly or slowly the function is increasing or decreasing at a particular point in its domain.'}.
...
In this sense, we would say the [limit of the function]{.concept entry="Limit of a function" definition="The limit of $f(x)$ as $x$ approaches $c$, denoted $\lim_{x \to c} f(x)$, is the value that $f(x)$ gets closer and closer to as $x$ gets closer and closer to $c$ without reaching it.  The formal definition is that $\lim_{x \to c} f(x) = y$ if, for all $\epsilon > 0$, there is a value $\delta > 0$ such that $|f(x) - y| < \epsilon$ for all $x \in (c - \delta, c) \cup (c, c + \delta)$.  A function may not have a limit at a particular point."} as $x$ approaches 0 is equal to 1.
...
We would say that the function's [left-hand limit]{.concept entry="Left- and right-handed limits" definition="The left-handed limit is the value that $f(x)$ gets closer and closer to as $x$ gets closer and closer to $c$ without meeting or exceeding it.  The right-handed limit is the same thing, from the opposite direction."} at $x = 0$ is 0 and that its [right-hand limit]{.concept} at $x = 0$ is 1.
...
:::: {.answer title="Answer: right-hand"}
...
By contrast, in the parts of the domain where there are no such jumps, the function is [continuous]{.concept entry="Continuity" definition="A function $f : X \to Y$ is continuous at a point in its domain, $c \in X$, if its limit at that point exists and equals the value of the function: $\lim_{x \to c} f(x) = f(c)$.  We call $f$ a continuous function if it is continuous at every point in its domain."}.
...
Polynomials
: Any function of the form $$\begin{aligned}f(x) &= c_0 + c_1 x + c_2 x^2 + \cdots c_k x^k \\ &= \sum_{n=0}^k c_n x^n,\end{aligned}$$ where $k$ is a natural number and each $c_n$ is a real number, is called a [polynomial]{.concept definition="A polynomial is a function of the form $$f(x) = c_0 + c_1 x + c_2 x^2 + \cdots + c_k x^k,$$ where $k$ is a natural number and each coefficient $c_0, \ldots, c_k$ is a real number."} and is continuous.
...
: Any function of the form $f(x) = g(h(x))$, called the [composition]{.concept definition="Consider two functions $h : X \to Y$ and $g : Y \to Z$.  The composition of $g$ and $h$ is the function $f : X \to Z$ defined by $f(x) = g(h(x))$."} of $g$ and $h$, is continuous if $g$ and $h$ are continuous.
...
:::: {.answer title="Answer that uses @exr-lottery-limit"}
...
Specifically, the [slope]{.concept definition="For a linear function, $f(x) = \alpha + \beta x$, the slope is the coefficient $\beta$.  It can be calculated using the rise-over-run formula: for any distinct points $x_1$ and $x_2$ in the domain of $f$, $$\beta = \frac{f(x_1) - f(x_2)}{x_1 - x_2}.$$"} of $g(x)$ is greater in magnitude than that of $h(x)$.
...
This is the [constant multiple rule]{.concept definition="If $g(x) = c f(x),$ where $c$ is a constant real number, then $g'(x) = c f'(x).$"}.
...
You'll end up using the [sum rule]{.concept definition="If $h(x) = f(x) + g(x),$ then $h'(x) = f'(x) + g'(x).$"} so often that you stop thinking about it.
...
The [product rule]{.concept definition="If $h(x) = f(x) g(x),$ then $h'(x) = f(x) g'(x) + f'(x) g(x).$"} to take the derivative of $f(x) g(x)$ essentially breaks the problem into two parts.
...
The [quotient rule]{.concept definition="If $h(x) = f(x) / g(x),$ then $$h'(x) = \frac{f'(x) g(x) - f(x) g'(x)}{g(x)^2}.$$"} for functions in the form $\frac{f(x)}{g(x)}$ is unfortunately a bit trickier to remember.
...
The [chain rule]{.concept definition="If $h(x) = f(g(x)),$ then $h'(x) = f'(g(x)) \cdot g'(x).$"} helps us deal with situations like this one.
...
But now we're thinking about the rate of change in a rate of change ... or the derivative of a derivative, also known as the [second derivative]{.concept definition="The second derivative of a differentiable function $f(x)$, denoted $f''(x),$ is the derivative of the function's derivative: $$f''(c) = \lim_{x \to c} \frac{f'(x) - f'(c)}{x - c}.$$"}.
...
When it does, we say that $f$ is [twice differentiable at the point]{.concept entry="Twice differentiability" definition="A function $f : X \to \mathbb{R}$ is twice differentiable at a point in its domain, $c \in X,$ if the second derivative $f''(c)$ exists.  We call $f$ a twice differentiable function if it is twice differentiable at every point in its domain."} $c$.
...
If a function has a non-negative second derivative, $f''(x) \geq 0,$ we call it [convex]{.concept entry="Convex function" definition="A twice differentiable function is convex if $f''(x) \geq 0,$ and additionally is strictly convex if $f''(x) > 0.$"}.
...
On the other hand, we call a function [concave]{.concept entry="Concave function" definition="A twice differentiable function is concave if $f''(x) \leq 0,$ and additionally is strictly concave if $f''(x) < 0.$"} if $f''(x) \leq 0,$ and additionally [strictly concave]{.concept} if $f''(x) < 0.$
