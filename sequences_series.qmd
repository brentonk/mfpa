...
::: {#nte-summation .callout-note title="Summation notation"}
...
We just have to use a [logarithm]{.concept definition="The reverse of an exponent.  In particular, for any numbers $a > 0$ and $b > 0$, if $a^x = b$, then we say that $x = \log_a b$.  In this example, $a$ is called the base of the logarithm.  For example, $\log_{10} 1000 = 3$ because $10^3 = 1000$."}, which is essentially the reverse of an exponent.
...
A [sequence]{.concept definition="An infinite, ordered list of numbers typically denoted $\\{x_n\\}$."} is an infinite, ordered list of numbers.
...
Indeed, we would say the [limit]{.concept definition="A point that a sequence $\\{x_n\\}$ approaches as $n$ grows larger and larger.  To denote $x$ being the limit of $\\{x_n\\}$, we may write $\lim_{n \to \infty} x_n = x$ or $\\{x_n\\} \to x$.  Some sequences have no limit.  See @def-limit for a fully rigorous mathematical definition of a limit."} of the sequence is 0, which we'd denote by writing $\lim_{n \to \infty} x_n = 0$ or, more simply, $\{x_n\} \to 0$.
...
The sequence is [convergent]{.concept entry="Convergent sequence" definition="A sequence that has a finite limit."} if and only if $\lim_{n \to \infty} x_n = x$ for some finite number $x$.
:::
...
We call a sequence [divergent]{.concept entry="Divergent sequence" definition="A sequence that has no finite limit, either because it has an infinite limit or because it oscillates."} if it is not convergent.
...
In fact, it turns out that a necessary condition for a sequence to converge is that the sequence be [bounded]{.concept entry="Bounded sequence" definition="A sequence for which there is a finite number $B$ such that every $|x_n| \leq B$.  Every convergent sequence is bounded, but not every bounded sequence is convergent."}.
...
A [monotone]{.concept entry="Monotone sequence" definition="A sequence whose values always increase (every $x_{n+1} \geq x_n$) or always decrease (every $x_{n+1} \leq x_n$)."} sequence, in which the values always increase (every $x_{n+1} \geq x_n$) or always decrease (every $x_{n+1} \leq x_n$), always converges if it is bounded.
...
:::: {.answer title="Part (a): Constant multiple of sequence"}
...
:::: {.answer title="Part (b): Sum of sequences"}
...
:::: {.answer title="Part (c): Difference of sequences"}
...
:::: {.answer title="Part (d): Product of sequences"}
We need to prove that for all $\epsilon > 0$, there is an index $N$ such that $|x_n y_n - x y| < \epsilon$ for all $n \geq N$.
To this end, take an arbitrary $\epsilon > 0$.
...
:::: {.answer title="Part (e): Ratio of sequences"}
Assume $y \neq 0$.
...
But how do we calculate an infinite sum, or [series]{.concept definition="A summation of infinitely many terms, written $\sum_{k=1}^\infty a_k$.  Its value is the limit of the sequence of partial sums, $\lim_{n \to \infty} \sum_{k=1}^n a_k$, provided that this limit exists."}, like this one?
...
In particular, we will work with the sequence of [partial sums]{.concept entry="Partial sum" definition="The sum of all terms in a series up to a specified, finite index."}, each of which is the sum of the first $n$ terms of the series.
...
If the limit exists and is finite, we call the series [convergent]{.concept entry="Convergent series" definition="A series whose value exists and is finite."}.
:::

Possibly the most famous example of a convergent series is the [geometric series]{.concept definition="A series of the form $\sum_{k=0}^\infty q^k = 1 + q + q^2 + \cdots$, where $0 < q < 1$.  A geometric series is convergent, with a value of $\frac{1}{1 - q}$."}, a sum of powers of a fraction between 0 and 1.
