...
Our reasons for starting with set theory are related, but more prosaic.
...
#.  One of the things you'll get from this course is a basic introduction to mathematical proofs.
    Set theory gives us some nice examples of mathematical statements that are relatively easy to prove, making it a convenient starting point for our study of proofs.
...
You would read this as "$A$ is the set of natural numbers $x$ such that $x$ is greater than 1000".^[I always use the vertical bar $\mid$ when writing in set-builder notation, but sometimes you will see people use the colon $:$ instead.  These are just different conventions with no differences in underlying meaning.]
...
$A \in B$ means "the set $A$ is, itself, an element of $B$".^[If all the elements of $B$ are sets, we would typically call $B$ a [collection]{.concept}.]
...
I'm bringing up this seemingly obvious fact mainly because it'll give us a nice first exposure to mathematical proofs.^[And secondarily because some seemingly obvious things turn out not to be true at all!  This is one of the reasons it's important to prove our mathematical claims.]
...
In other words, none of these types of results is more or less true than the others.
The different labels are mostly used to help readers decode the context and importance of each type of result.

- [Theorem:]{.concept} Reserved for results that are especially big, important, fundamental, general, etc.

- [Proposition:]{.concept} A bread-and-butter result.  Useful and important enough to care about on its own, but not earth-shaking enough to be a theorem --- like @prp-subset-transitive here!
...
I find myself in this situation often, and as of summer 2025 I've found that ChatGPT's `o3-mini-high` model gives great explanations when I'm stuck on something mathematical.
...
As of early 2025, the [Quarto](https://quarto.org) software I'm using to write these notes doesn't insert any of these symbols automatically, so you'll just have to take the bottom of the box that the proof lives in as my "QED" variant.
::::
...
Like Dr. Frankenstein, we can create new sets from the pieces of old sets!
Feel the power!

OK, maybe it's not that groundbreaking or cool.
But we do want to be able to talk about combinations of sets.
...
::: {.callout-warning}
## The mathematical "or"
In mathematical and logical writing, "or" has the same meaning as "and/or" in ordinary writing.
For example, when I say "$x \in A$ or $x \in B$" in the definition of $A \cup B$, that applies to all of the following situations:
...
::: {.proof}
There are two claims to prove here.^[There is also the third, implicit claim that $A \cap B \subseteq A \cup B$, but this follows from @prp-subset-transitive once we've proved that $A \cap B \subseteq A$ and $A \subseteq A \cup B$.]
...
2.  We know from @prp-intersection-subset-union that $A \cap \emptyset \subseteq \emptyset$.
    ...
    (Alternatively, see @exm-emptyset-subset in the next chapter for a more direct argument that $\emptyset \subseteq A \cap \emptyset$.)
::::
:::
...
To see why there are more real numbers than natural numbers, we will sketch out [the diagonal argument](https://en.wikipedia.org/wiki/Cantor%27s_diagonal_argument) from the 19th century mathematician Georg Cantor.^[It's a sketch because there are some nagging details that we will ignore.  For example, we are ignoring the possibility of two different decimal expansions corresponding to the same number, as is the case with 0.1 and 0.099999$\cdots$.]
