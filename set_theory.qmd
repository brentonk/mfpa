...
Seemingly every book on "higher math" --- the kind of math you encounter after the intro calculus sequence in college --- starts with a discussion of [set theory]{.concept definition="The study of sets and their properties.  A key logical foundation for much of the rest of mathematics."}.
...
::: {#def-set}
## Sets and elements
A [set]{.concept definition="Informally, a collection of objects (not necessarily numbers).  A set $S$ with elements $a$, $b$, and $c$ is written $S = \\{a, b, c\\}$."} is a collection of objects, which we call the [elements]{.concept entry="Element" definition='A member of a set.  We write $a \in A$ to mean "$a$ is an element of the set $A$."'} of the set.
...
::: {#def-subset}
## Subset
The set $A$ is a [subset]{.concept definition="When all of the elements of the set $A$ are also elements of the set $B$, we say that $A$ is a subset of $B$, written $A \subseteq B$."} of the set $B$, denoted $A \subseteq B$, if every element of $A$ is an element of $B$.
:::
...
The [union]{.concept definition="The union of the sets $A$ and $B$, denoted $A \cup B$, is the set of all elements that are in at least one of $A$ or $B$."} of the sets $A$ and $B$, denoted $A \cup B$, is the set of elements that are in $A$, in $B$, or both: $$A \cup B = \{x \mid x \in A \text{ or } x \in B\}.$$
The [intersection]{.concept definition="The intersection of the sets $A$ and $B$, denoted $A \cap B$, is the set of all elements that are in both $A$ and $B$."} of the sets $A$ and $B$, denoted $A \cap B$, is the set of elements that are in both $A$ and $B$: $$A \cap B = \{x \mid x \in A \text{ and } x \in B\}.$$
:::
...
::: {#def-set-difference}
## Set difference
The [set difference]{.concept definition="The set difference between $A$ and $B$, denoted $A \setminus B$, is the set of all elements that are in $A$ and are not in $B$."} between sets $A$ and $B$, denoted $A \setminus B$, is the set of elements that are in $A$ and not in $B$: $$A \setminus B = \{a \in A \mid a \notin B\}.$$
:::
...
::: {#def-empty-set}
## Empty set
The [empty set]{.concept definition="The set containing no elements, denoted $\emptyset$."} is the set containing no elements, denoted $\emptyset$.
:::
