...
But we'll study logic and proof writing more in depth at the end of the course, where you'll see that set-theoretic statements are a great place to start building your argument-stating and proof-writing skills.
...
: {tbl-colwidths="[20,10,10,12,12,12,12]"}
...
::: {#exr-complement name="Complement of a set"}
...
::: {#exr-complement-repeated name="Complement of the complement"}
...
X^c \cap Y^c &= \{3, 5, 6, 7, 9, 10\} \cap \{1, 3, 5, 7, 9, 10\} \\ &= \{3, 5, 7, 9, 10\}.
\end{aligned}
$$
...
- Every US president took office in their first term in some year: George Washington in 1789, Grover Cleveland in 1885, Barack Obama in 2009, and so on.
...
Then we have $y(\text{George Washington}) = 1789,$ $y(\text{Grover Cleveland}) = 1885,$ and so on.
...
In mathematical terms, we would call the president-and-their-vice-president(s-if-any) association a [relation](https://en.wikipedia.org/wiki/Relation_(mathematics)).
...
If the function weren't one-to-one, then there'd be at least one $b \in B$ with multiple matches in $A.$
...
= f(x).
$$
It will be easier to see why this is useful when we get to the chain rule in calculus.
...
1. $f : S \to \mathbb{N}$, where $S$ is the set of US states and $f(s)$ represents the age of the oldest member of the state's House delegation.
